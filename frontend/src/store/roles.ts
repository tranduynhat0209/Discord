...
export const getRole = (id: string) =>
    createSelector(
        ...
        (roles: any) => roles.find((r) => r.id === id)
    );
...
export const getRoleByName = (guildId: string, name: string) =>
    createSelector(
        (state) => state.entities.roles,
        (roles: any) =>
            roles.find((r) => r.guildId === guildId && r.name === name)
    );

export const getRoles = (ids: string[]) =>
    createSelector(
        (state) => state.entities.roles,
        (roles: any) => roles.filter((r) => ids.includes(r.id))
    );

export const filterHoistedRoles = (guildId: string) =>
    createSelector(
        (state) => state.entities.roles,
        (roles: any) => roles.filter((r) => r.guildId === guildId && r.hoisted)
    );
