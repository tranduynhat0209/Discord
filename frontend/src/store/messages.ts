...
const slice = createSlice({
  ...
  reducers: {
    ...
    created: ({ list, total }, { payload }: Action<WS.Args.MessageCreate>) => {
      list.push(payload.message);
      total[payload.message.channelId]++;
    },
    deleted: ({ list, total }, { payload }: Action<WS.Args.MessageDelete>) => {
...
export const fetchMessages = (channelId: string, back = 25) => (dispatch, getState: () => AppState) => {
